"""
    ...
    The response format is not so ReSTful... Because there have been so many
    project using `status` field as return code, HTTP status code should always
    200.
...
class ProcessingException(HTTPException):
    ...
    `status` is the status code of the response supplied to the client in
    the case that this exception is raised. `msg` is an error message
    ...
    def __init__(self, msg='', status=RespCode.UNKNOWN_ERROR,
                 *args, **kwargs):
...
def catch_processing_exceptions(func):
    ...
    def decorator(*args, **kw):
        try:
            ...
            msg = exception.msg or str(exception)
...
class ModelView(MethodView):
    ...
    def query(self, model=None):
        """Returns either a SQLAlchemy query or mongodb query object
...
class FunctionAPI(ModelView):
    ...
    def get(self):
        ...
        try:
            ...
            current_app.logger.exception(str(exception))
            return {'msg': 'Unable to decode data',
        ...
        try:
            ...
            return {'status': RespCode.DATA_ERROR, 'msg': message}
...
class API(ModelView):
    ...
    def __init__(self, model, exclude_columns=None,
    ...
    def _handle_validation_exception(self, exception):
        ...
        return {'status': RespCode.DATA_ERROR, 'msg': errors}
    ...
    def _instid_to_dict(self, instid):
        ...
        If no such instance of the model exists, this method aborts.
    ...
    def _search(self):
        ...
        try:
            ...
            current_app.logger.exception(str(exception))
            return {'status': RespCode.DATA_ERROR, 'msg': "can't decode data!"}
        ...
        # NOTE: maybe we can modify here directly...
    ...
    def get(self, instid, relationname, relationinstid):
        ...
        if instance is None:
            return {'msg': 'no specified model', 'status': RespCode.DATA_ERROR}
        ...
        if relationname is None:
            ...
            if relationinstid is not None:
                ...
                if related_value_instance is None:
                    return {'msg': 'related value instance not exist',
                            'status': RespCode.DATA_ERROR}
        ...
        if result is None:
            return {'msg': 'no result!',
                    'status': RespCode.DATA_ERROR}
        result = {'status': RespCode.OK, 'data': result}
