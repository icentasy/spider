    ...
    def __init__(self, db):
        ...
        if isinstance(db_config, dict):
            for route_key in db_config:
                ...
                if isinstance(db_config[route_key], str):
                    engine_dict = {'master': sqlalchemy.create_engine(db_config[route_key]),
                                   'slave': sqlalchemy.create_engine(db_config[route_key])}
                    self._bind_maps[route_key].append(engine_dict)
                elif isinstance(db_config[route_key], dict):
                    try:
                        engine_dict = {'master': sqlalchemy.create_engine(db_config[route_key]['master']),
                                       'slave': sqlalchemy.create_engine(db_config[route_key]['slave'])}
                    except TypeError as e:
                        raise DbConfigError
                    self._bind_maps[route_key].append(engine_dict)
                elif isinstance(db_config[route_key], list):
                    for config_dict in db_config[route_key]:
                        try:
                            engine_dict = {'master': sqlalchemy.create_engine(config_dict['master']),
                                           'slave': sqlalchemy.create_engine(config_dict['slave'])}
                        except TypeError as e:
                            raise DbConfigError
                        self._bind_maps[route_key].append(engine_dict)
        else:
            self._bind_maps['default'] = sqlalchemy.create_engine(db_config, encoding='utf-8', echo=debug_config)
...
class ShardedSession(SessionBase):
    ...
    def __init__(self, db, autocommit=True, autoflush=True, query_cls=ShardedQuery, **options):
        self.db = db
        super(ShardedSession, self).__init__(autocommit=True, autoflush=autoflush,
                                             bind=None,
                                             binds={},
                                             query_cls=query_cls,
                                             **options)
        ...
        def query_chooser(query):
            ids = []
            try:
                shard_key = query._mapper_zero().mapped_table.info.get('shard_key', None)
                route_key = query._mapper_zero().mapped_table.info.get('route_key', None)
            ...
            if shard_key is None:
                return [0]
            engine_counts = query.session.db.get_engine_counts_by_route(route_key)
            shard_lookup_func = query._mapper_zero().mapped_table.info.get('shard_lookup', None)
            ...
            for column, operator, value in _get_query_comparisons(query):
                if column.key == shard_key:
                    if operator == operators.eq:
                        ...
                        ids.extend(shard_lookup_func(v, engine_counts) for v in value)
...
class ArmoryOrm(object):
    ...
    def init_conf(self, conf):
        '''
        conf should be like:
        {"test":[{"master":"mysql://vpn:123456@10.164.18.108:3306/access", "slave":"mysql://vpn:123456@10.164.130.41:3306/access"}]}
        '''
...
if __name__ == '__main__':
    ...
    class spots(orm.Model):
        ...
        def __init__(self, square, password, bssid, ssid, x, y, updatetime, is_shared, location):
            self.square = square
