...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/var/app/enabled/auth_service
DAEMON=/usr/local/bin/uwsgi
NAME=AuthService
DESC="Auth Service uWSGI Web Server"
...
function start_uwsgi()
{
	if [ -f "$PID_FILE" ]; then
	    ...
	    pushd ${APP_DIR}/auth_service >/dev/null
