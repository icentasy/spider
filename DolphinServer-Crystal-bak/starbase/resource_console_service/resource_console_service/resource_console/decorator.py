    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if EXCEPTION_DEBUG:
                ...
                return json_response_error(respcode, e.msg)
    return wrapper
...
def perf_logging(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            _LOGGER.debug('%s <- %s ms.' % (msg, proc_time))
        except Exception as e:
            end_time = time.time()
            proc_time = round(end_time - start_time, 3)
            _LOGGER.error('%s error in %s ms.' % (msg, proc_time))
            raise e
        return ret
    return wrapper
