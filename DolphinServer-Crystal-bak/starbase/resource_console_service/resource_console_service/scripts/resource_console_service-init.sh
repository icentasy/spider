...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/var/app/enabled/resource_console
DAEMON=/usr/local/bin/uwsgi
NAME=ResourceConsole
DESC="Resource Console Service uWSGI Web Server"
PROJECT=resource_console_service
SPIDERS=1
...
function start_uwsgi()
{
	if [ -f "$PID_FILE" ]; then
	    ...
	    pushd ${APP_DIR}/resource_console >/dev/null
